...
Este repositório contém a interface gráfica (Front-end) desenvolvida em **React** para a aplicação de blogging dinâmico, referente à Fase 03 do Tech Challenge da pós-graduação em Full Stack Development.
...
*   **Vite:** Ferramenta de build super rápida utilizada para inicializar e empacotar o projeto.
...
*   **`/components`**: Contém componentes visuais reaproveitáveis e estruturais.
    *   *Exemplo:* `PrivateRoute.jsx` (componente que barra o acesso a rotas não autorizadas).
*   **`/pages`**: Agrupa as telas completas mapeadas nas rotas da aplicação.
    *   *Exemplo:* `Home.jsx`, `PostView.jsx`, `Admin.jsx`, `CreatePost.jsx`, etc.
*   **`/services`**: Responsável pela configuração de comunicação com o mundo externo.
    *   *Exemplo:* `api.js` (instância configurada do Axios com a URL base do back-end e interceptadores de token).
*   **`/context`**: Armazena a lógica de estado global da aplicação.
    *   *Exemplo:* `AuthContext.jsx` (provedor de dados sobre o login do usuário).
...
---
*Desenvolvido como requisito da Fase 03 da pós-graduação em Full Stack Development.*